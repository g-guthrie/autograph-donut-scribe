...
const waitForPlayerCount = async (expectedMaxPlayers) => {
  const deadline = Date.now() + 60_000
  while (Date.now() < deadline) {
    const status = await fetchStatus().catch(() => null)
    if (status && (status.players ?? 0) <= expectedMaxPlayers) {
      return status
    }
    await sleep(1000)
  }
  throw new Error(`player count never dropped to <= ${expectedMaxPlayers}`)
}

const joinViaTab = async (page, tabIndex) => {
  await page.evaluate((index) => {
    window.__cs15TestMenuDriver.openMultiplayer()
    window.__cs15TestMenuDriver.openInternetGame()
    window.__cs15TestMenuDriver.setInternetGameTab(index)
...
await joinPage.waitForFunction(
  () => (window.__cs15TestState?.serverListCount ?? 0) >= 1,
  {timeout: 60_000},
)
await joinPage.evaluate(() => {
  window.__cs15TestMenuDriver.selectInternetGameRow(0)
  window.__cs15TestMenuDriver.toggleFavorite()
  window.__cs15TestMenuDriver.saveBrowserLists()
  window.__cs15TestMenuDriver.joinInternetGame()
})
await waitForSession(joinPage, 'initial-join', consoleMessages, 2, hostResult.status.txid)

await joinPage.close()
await waitForPlayerCount(1)

const favoritesPage = await openClient(browser, dialogs, consoleMessages)
await joinViaTab(favoritesPage, 2)
await waitForSession(favoritesPage, 'favorites-reopen', consoleMessages, 2, hostResult.status.txid)

await favoritesPage.close()
await waitForPlayerCount(1)

const historyPage = await openClient(browser, dialogs, consoleMessages)
await joinViaTab(historyPage, 3)
await waitForSession(historyPage, 'history-reopen', consoleMessages, 2, hostResult.status.txid)

if (dialogs.length) {
  throw new Error(`unexpected dialogs: ${dialogs.join(' | ')}`)
}

console.log(`favorites/history persistence test passed: txid=${hostResult.status.txid}`)
