...
const waitForClientSession = async (page, label, consoleMessages, minPlayers, expectedTxid = null) => {
  ...
  throw new Error(`${label}: never reached backend-ready session: ${JSON.stringify(lastSnapshot)}; console=${JSON.stringify(recentConsole)}`)
}
...
const hostResult = await waitForClientSession(hostPage, 'host', consoleMessages, 1, hostTxid)

const joinPage = await openClient(browser, dialogs, consoleMessages)
await joinSandboxRoom(joinPage)
const joinResult = await waitForClientSession(joinPage, 'joiner', consoleMessages, 2, hostResult.status.txid)

if (hostResult.status.txid !== joinResult.status.txid) {
  throw new Error(`clients ended in different sessions: host=${hostResult.status.txid} joiner=${joinResult.status.txid}`)
}
if (hostResult.status.map !== joinResult.status.map) {
  throw new Error(`clients ended on different maps: host=${hostResult.status.map} joiner=${joinResult.status.map}`)
}
if (dialogs.length) {
  throw new Error(`unexpected dialogs: ${dialogs.join(' | ')}`)
}

console.log(`internet game join test passed: txid=${joinResult.status.txid} map=${joinResult.status.map} players=${joinResult.status.players}`)
