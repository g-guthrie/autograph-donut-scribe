...
const waitForSandboxHost = async (page, consoleMessages) => {
  ...
  throw new Error(`sandbox host never reached backend-ready state: ${JSON.stringify(lastSnapshot)}; console=${JSON.stringify(recentConsole)}`)
}
...
const {state, status} = await waitForSandboxHost(page, consoleMessages)
if (!status?.txid) {
  throw new Error('missing sandbox transaction id')
}
...
console.log('sandbox host test passed')
