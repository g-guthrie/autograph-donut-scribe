...
#define SANDBOX_DEFAULT_HOSTNAME "CS1.5 Sandbox"
...
static bool g_bSandboxCreateGame = false;

#if defined(__EMSCRIPTEN__)
EM_JS( int, UI_RequestSandboxHost, (
	const char *mapName,
	const char *hostname,
	const char *password,
	int maxPlayers,
	char *outConnectAddress,
	int outConnectAddressSize,
	char *outProtocol,
	int outProtocolSize
), {
	const txid = window.__cs15PrepareHostTx ? window.__cs15PrepareHostTx() : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
	window.__cs15TraceUpdate?.({
		txid,
		stage: 'sandbox_host_request',
	});

	const payload = JSON.stringify({
		txid,
		map: UTF8ToString( mapName ),
		hostname: UTF8ToString( hostname ),
	...
	try
	{
		...
		xhr.open( "POST", "http://127.0.0.1:27020/sandbox-host", false );
		xhr.setRequestHeader( "Content-Type", "application/json" );
		xhr.send( payload );
		if( xhr.status >= 200 && xhr.status < 300 )
		{
			const response = JSON.parse( xhr.responseText );
			const connectAddress = `${response.connect_host}:${response.connect_port}`;
			const protocol = String( response.protocol ?? "49" );
			stringToUTF8( connectAddress, outConnectAddress, 128 );
			stringToUTF8( protocol, outProtocol, 16 );
			window.__cs15TraceUpdate?.({
				txid: response.txid ?? txid,
				stage: response.stage,
				lastCompletedStage: response.last_completed_stage ?? response.stage,
				stageTimes: response.stage_times ?? {},
		...
		window.__cs15TraceUpdate?.({
			txid,
			lastError: `sandbox-host ${xhr.status}`,
		});
		console.error( "[sandbox-host]", xhr.status, xhr.responseText );
	}
	catch( error )
	{
		window.__cs15TraceUpdate?.({
		...
		console.error( "[sandbox-host]", error );
	}
...
void CMenuCreateGame::Begin( CMenuBaseItem *pSelf, void *pExtra )
{
	...
	if( g_bSandboxCreateGame )
	{
		int maxPlayers = atoi( menu->maxClients.GetBuffer() );
		if( maxPlayers < 2 )
			maxPlayers = 16;
		else if( maxPlayers > 32 )
			maxPlayers = 32;

		const char *hostname = menu->hostName.GetBuffer()[0] ? menu->hostName.GetBuffer() : SANDBOX_DEFAULT_HOSTNAME;
		const char *password = menu->password.GetBuffer();

#if defined(__EMSCRIPTEN__)
		char connectAddress[128];
		char protocol[16];
		connectAddress[0] = '\0';
		protocol[0] = '\0';
		if( !UI_RequestSandboxHost( mapName, hostname, password, maxPlayers, connectAddress, sizeof( connectAddress ), protocol, sizeof( protocol )))
		{
			UI_ShowMessageBox( L( "Failed to start sandbox server" ));
			return;
		}

		if( !connectAddress[0] )
			Q_strncpy( connectAddress, "127.0.0.1:8080", sizeof( connectAddress ));
		if( !protocol[0] )
			Q_strncpy( protocol, "49", sizeof( protocol ));

		EngFuncs::PlayBackgroundTrack( NULL, NULL );
		EngFuncs::ClientCmdF( FALSE, "disconnect;menu_connectionprogress menu %s;connect \"%s\" \"%s\"\n", connectAddress, connectAddress, protocol );
		return;
#endif
	}
	...
	const char *sandboxCommands =
		g_bSandboxCreateGame ?
		"mp_freeforall 1;mp_autoteambalance 0;mp_limitteams 0;mp_auto_join_team 1;humans_join_team any;mp_freezetime 0;mp_round_infinite abcdefghijk;mp_roundrespawn_time -1;mp_forcerespawn 1;" :
		"";
	snprintf( cmd, sizeof( cmd ), "disconnect;menu_connectionprogress localserver;wait;wait;wait;maxplayers %i;latch;%smap %s\n", atoi( menu->maxClients.GetBuffer() ), sandboxCommands, cmd2 );
...
ADD_MENU3( menu_creategame, CMenuCreateGame, UI_CreateGame_Menu );

void UI_CreateGame_Menu( void )
{
	g_bSandboxCreateGame = false;
	menu_creategame->Show();
}

void UI_Sandbox_Menu( void )
{
	g_bSandboxCreateGame = true;
	EngFuncs::CvarSetValue( "public", 1.0f );
	EngFuncs::CvarSetString( "hostname", SANDBOX_DEFAULT_HOSTNAME );
	EngFuncs::CvarSetString( "sv_password", "" );
	menu_creategame->Show();
}

static void UI_MenuSandbox_f( void )
{
	UI_Sandbox_Menu();
}

static void UI_MenuTestSelectMap_f( void )
{
	if( !menu_creategame )
		return;

	if( !menu_creategame->WasInit() || !menu_creategame->IsVisible() )
		return;

	const char *selection = EngFuncs::CmdArgv( 1 );
	if( !selection[0] )
		return;
	...
	if( end && *end == '\0' )
	{
		menu_creategame->mapsList.SetCurrentIndex( (int)index );
		return;
	}

	for( int i = 0; i < menu_creategame->mapsListModel.Count(); ++i )
	{
		if( !V_stricmp( menu_creategame->mapsListModel[i].name, selection ))
		{
			menu_creategame->mapsList.SetCurrentIndex( i );
			return;
		}
	}
}

static void UI_MenuTestOk_f( void )
{
	if( !menu_creategame || !menu_creategame->done )
		return;
	if( !menu_creategame->WasInit() || !menu_creategame->IsVisible() )
		return;

	CMenuCreateGame::Begin( menu_creategame->done, nullptr );
}

ADD_COMMAND( menu_sandbox, UI_MenuSandbox_f );
ADD_COMMAND( menu_test_selectmap, UI_MenuTestSelectMap_f );
ADD_COMMAND( menu_test_ok, UI_MenuTestOk_f );
