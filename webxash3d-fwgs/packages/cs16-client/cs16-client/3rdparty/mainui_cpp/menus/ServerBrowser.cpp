...
#if defined(__EMSCRIPTEN__)
EM_JS( int, UI_FetchBackendServerEntry, (char *outAddress, int outAddressSize, char *outInfo, int outInfoSize), {
	try
	{
		const xhr = new XMLHttpRequest();
		xhr.open( "GET", "http://127.0.0.1:27016/status", false );
		xhr.send();
		...
		const connectAddress = `${status.connect_host}:${status.connect_port}`;
		const slash = String.fromCharCode( 92 );
		const info = [
			"",
			"p", String( status.protocol ?? "49" ),
			"host", String( status.hostname ?? "CS1.5 Sandbox" ),
			"map", String( status.map ?? "de_dust2" ),
			"dm", "1",
			"team", "0",
			"coop", "0",
			"numcl", String( status.players ?? 0 ),
			"maxcl", String( status.max_players ?? 0 ),
			"gamedir", "cstrike",
			"password", status.password ? "1" : "0",
		].join( slash );

		stringToUTF8( connectAddress, outAddress, outAddressSize );
		stringToUTF8( info, outInfo, outInfoSize );
		return 1;
	}
	catch( error )
	{
		console.error( "[backend-server-entry]", error );
		return 0;
	}
} );
#endif
...
void CMenuServerBrowser::RefreshList()
{
	...
	else if( uiStatic.realTime > refreshTime2 )
	{
		...
		{
#if defined(__EMSCRIPTEN__)
			char backendAddress[128];
			char backendInfo[512];
			backendAddress[0] = '\0';
			backendInfo[0] = '\0';
			if( UI_FetchBackendServerEntry( backendAddress, sizeof( backendAddress ), backendInfo, sizeof( backendInfo )))
			{
				netadr_t adr;
				if( EngFuncs::textfuncs.pNetAPI->StringToAdr( backendAddress, &adr ))
					AddServerToList( adr, backendInfo );
			}
			else
#endif
			{
...
static void UI_MenuTestRefresh_f( void )
{
	if( !menu_internetgames || !menu_internetgames->WasInit() || !menu_internetgames->IsVisible() )
		return;

	menu_internetgames->RefreshList();
}

static void UI_MenuTestSelectServer_f( void )
{
	if( !menu_internetgames || !menu_internetgames->WasInit() || !menu_internetgames->IsVisible() )
		return;

	int index = atoi( EngFuncs::CmdArgv( 1 ));
	if( !menu_internetgames->gameListModel.servers.IsValidIndex( index ))
		return;

	menu_internetgames->gameList.SetCurrentIndex( index );
	menu_internetgames->OnChangeSelectedServer();
}

static void UI_MenuTestJoin_f( void )
{
	if( !menu_internetgames || !menu_internetgames->WasInit() || !menu_internetgames->IsVisible() )
		return;
	if( !menu_internetgames->gameListModel.servers.IsValidIndex( menu_internetgames->gameList.GetCurrentIndex() ))
		return;

	menu_internetgames->JoinGame();
}

static void UI_MenuTestBrowserTab_f( void )
{
	if( !menu_internetgames || !menu_internetgames->WasInit() || !menu_internetgames->IsVisible() )
		return;

	menu_internetgames->tabSwitch.SetState( bound( 0, atoi( EngFuncs::CmdArgv( 1 )), 3 ));
	menu_internetgames->OnTabSwitch();
}

static void UI_MenuTestAddServer_f( void )
{
	if( !menu_internetgames || !menu_internetgames->WasInit() || !menu_internetgames->IsVisible() )
		return;

	const char *address = EngFuncs::CmdArgv( 1 );
	const char *protocol = EngFuncs::CmdArgv( 2 );
	if( !address[0] )
		return;

	menu_internetgames->addressField.SetBuffer( address );
	if( !strcmp( protocol, "48" ) || !strcmp( protocol, "legacy" ))
		menu_internetgames->serverProtocol.SetCurrentValue( 1.0f );
	else if( !strcmp( protocol, "gs" ) || !strcmp( protocol, "goldsrc" ))
		menu_internetgames->serverProtocol.SetCurrentValue( 2.0f );
	else menu_internetgames->serverProtocol.SetCurrentValue( 0.0f );

	menu_internetgames->AddServer();
}

static void UI_MenuTestToggleFavorite_f( void )
{
	if( !menu_internetgames || !menu_internetgames->WasInit() || !menu_internetgames->IsVisible() )
		return;

	menu_internetgames->FavoriteServer();
}

static void UI_MenuTestSaveLists_f( void )
{
	if( !menu_internetgames || !menu_internetgames->WasInit() )
		return;

	menu_internetgames->SaveLists();
}

ADD_COMMAND( menu_test_refresh, UI_MenuTestRefresh_f );
ADD_COMMAND( menu_test_selectserver, UI_MenuTestSelectServer_f );
ADD_COMMAND( menu_test_join, UI_MenuTestJoin_f );
ADD_COMMAND( menu_test_browsertab, UI_MenuTestBrowserTab_f );
ADD_COMMAND( menu_test_addserver, UI_MenuTestAddServer_f );
ADD_COMMAND( menu_test_togglefavorite, UI_MenuTestToggleFavorite_f );
ADD_COMMAND( menu_test_savelists, UI_MenuTestSaveLists_f );
