...
static void CS15_CopyJsonString( char *dst, size_t dstSize, const char *src )
{
	if( !dstSize )
		return;

	size_t out = 0;
	for( size_t i = 0; src && src[i] && out + 1 < dstSize; ++i )
	{
		const char ch = src[i];
		if( ch == '"' || ch == '\\' || ch == '\n' || ch == '\r' || ch == '\t' )
		{
...
void __CmdFunc_CS15RuntimeState()
{
	const bool spectator = gEngfuncs.IsSpectateOnly() != 0;
	const bool hasSuit = ( gHUD.m_iWeaponBits & ( 1 << WEAPON_SUIT )) != 0;
	const bool hudVisible = ( gHUD.m_iHideHUDDisplay & HIDEHUD_ALL ) == 0 && hasSuit;
	const bool scoreboardVisible = gHUD.m_Scoreboard.ShouldDrawScoreboard();
	const bool playable = hudVisible && !spectator && !gHUD.m_iIntermission && gHUD.m_Health.m_iHealth > 0 && !gHUD.m_fPlayerDead;

	int playerNum = gHUD.m_Scoreboard.m_iPlayerNum;
	if( playerNum < 1 || playerNum > MAX_PLAYERS )
		playerNum = 0;

	const int team = playerNum ? g_PlayerExtraInfo[playerNum].teamnumber : 0;
	char serverName[96];
	char payload[256];

	CS15_CopyJsonString( serverName, sizeof( serverName ), gHUD.m_szServerName );
	snprintf(
		payload,
		sizeof( payload ),
		"cs15-runtime {\"playable\":%d,\"hudVisible\":%d,\"spectator\":%d,\"intermission\":%d,\"scoreboardVisible\":%d,\"hasSuit\":%d,\"health\":%d,\"team\":%d,\"serverName\":\"%s\"}\n",
		playable ? 1 : 0,
		hudVisible ? 1 : 0,
		spectator ? 1 : 0,
		gHUD.m_iIntermission ? 1 : 0,
		scoreboardVisible ? 1 : 0,
		hasSuit ? 1 : 0,
		gHUD.m_Health.m_iHealth,
		team,
		serverName
	);
...
void CHud :: Init( void )
{
	...
	HOOK_COMMAND_FUNC( "cs15_runtime_state", __CmdFunc_CS15RuntimeState, );
